...
router.get("/group/:groupId", async (req, res, next) => {
  ...
  try {
    const [expensesResponse, budgetsResponse] = await Promise.all([
      axios.get(`http://localhost:3000/api/expenses/group/${groupId}`, { headers: { Authorization: token }, timeout: 5000 }),
      axios.get(`http://localhost:3000/api/budgets/group/${groupId}`, { headers: { Authorization: token }, timeout: 5000 })
    ]);

    const aggregatedData = await aggregateGroupData(expensesResponse.data, budgetsResponse.data);
